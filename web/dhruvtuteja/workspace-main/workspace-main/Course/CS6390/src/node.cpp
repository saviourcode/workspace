...
struct Routing
{
    ...
    // Common Function
    void commonFunction(size_t, size_t, int (&)[NUMNODES][NUMNODES], function<void(int &)> func);

    // Common Function Helper: Remove
    void removeTmpTreePath(int &);
};
...
void Routing::removeTmpTreePath(int &tmpIntreeElem)
{
    tmpIntreeElem = 0;
}

void Routing::commonFunction(size_t ID, size_t rootedAt, int (&tmpIntree)[NUMNODES][NUMNODES], function<void(int &)> func)
{
    // Queue to traverse
...
void Routing::buildSPT(size_t ID, size_t rootedAt, int (&tmpIntree)[NUMNODES][NUMNODES])
{
    ...
    commonFunction(ID, rootedAt, tmpIntree, removeTmptreePath);
    ...
    // Prune the dead nodes from the intree by comparing it with the last tempintree
    {
        ...
        qTmpRmv.enqueue(rootedAt);

        // Mark the remove node as visited
        visRmv[rootedAt] = true;
...
void Node::computeIntree()
{
    ...
    while (oldpos = channel.input.tellg(), (line = readFile(channel.input)) != "")
    {
        ...
        // Merge the two trees
        msg.buildSPT(ID, rootedAt, tempIntree);
    }
